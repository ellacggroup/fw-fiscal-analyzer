...
function MethodologyPanel() {
  ...
  const sections = [
    ...
    {
      id: 'land-use-comparison',
      title: 'Fiscal Impact by Line Item: Current Zoning vs. Proposed Use',
      color: 'cyan',
      content: (
        <div className="space-y-4 text-sm text-gray-700">
          <p><strong>What this is:</strong> Every zoning or development-agreement item with a recognized land-use type shows a collapsible "Fiscal impact by line item" section below the official rating. For zoning changes with a parsed current→proposed designation, it shows two full breakdowns side by side — current zoning and proposed use — plus a Change column, each broken into police, fire/EMS, public works, parks, admin overhead, total cost, revenue, and net impact. This replaced an earlier version of this section that showed a single itemized estimate compared against a summary number; that design made it hard to tell how the two figures related to each other, so it was rebuilt to show the full current-vs-proposed math directly instead.</p>

          <p><strong>How each column is calculated:</strong> Each land-use type carries an assumed population, lane-mile, and parkland-acreage density per acre. Cost is that density × acreage × Fort Worth's per-capita/per-unit cost assumptions: $350/resident for police, $180/resident for fire and EMS, $800 per lane-mile for public works, $12,000 per acre for parks, plus 15% administrative overhead. This is applied identically to both the current zoning's land-use type and the proposed use's land-use type — same math, two scenarios — so the Change column is a real line-by-line comparison, not two different models forced together.</p>

          <p><strong>Revenue — current column:</strong> When a property address can be resolved for the item, the current column's revenue uses the parcel's <strong>actual current assessed value from the Tarrant Appraisal District</strong> × Fort Worth's property tax rate (0.7125%) — real data, not an estimate. This is marked with an "actual" badge next to the figure. When no address resolves or Tarrant Appraisal District has no record for the parcel, it falls back to Fort Worth's average per-acre revenue figure for the current zoning's land-use type, marked "est." instead.</p>

          <p><strong>Revenue — proposed column:</strong> Always an estimate, marked "est." — the proposed development hasn't been built yet, so there's no real assessed value to look up. For residential uses, a unit count is backed out of estimated population (÷ 2.4, an average household size) and multiplied by Fort Worth's average assessed value for that unit type ($280,000 single-family, $140,000 multifamily). For commercial, office, and industrial uses, it uses Fort Worth's average per-acre revenue figure for that land use — this bundles property tax with other revenue sources like sales tax, so it is not directly comparable to a single parcel's property tax bill. Deriving a building square footage from acreage alone would require inventing a floor-area-ratio assumption not backed by real data, so that path isn't used.</p>

          <p><strong>When there's no comparison column:</strong> Development Agreement items don't have a "current zoning" to compare against, and some Zoning Change items' current/proposed designations can't be parsed from the agenda text. In both cases, only the proposed-use breakdown is shown, with a note explaining why there's no current-zoning column.</p>

          <p><strong>A known limitation worth flagging:</strong> the current-zoning column depends on how well a zone code maps to one of the 8 land-use-type prototypes. Agricultural, floodplain, and unzoned/ETJ current zoning are all mapped to the "Open Space / Park" prototype, which carries a parkland-maintenance cost assumption intended for actual parks — this can overstate the current-zoning cost for raw undeveloped land, since the city isn't actually maintaining that acreage as parkland. Worth a closer look if a current-zoning cost figure looks unexpectedly high for vacant or agricultural land.</p>
        </div>
      ),
    },
    {
