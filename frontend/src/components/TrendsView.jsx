    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 text-sm text-gray-700">
          ...
          <p><strong>What it's compared against:</strong> The section shows a "Cost used for official rating" line so the two numbers can be checked against each other directly. For most categories that's the flat per-acre prototype cost. <strong>For zoning changes with a recognized from/to designation, the official rating is based on the incremental cost of the change — how much more it costs to serve the parcel after rezoning versus its current zoning — not the flat per-acre cost of the new use in isolation.</strong> That figure is usually smaller than the new use's standalone per-acre cost, since it nets out what the parcel already costs to serve today. The comparison line always reflects whichever figure actually drove the rating, so the two numbers on screen should always be checkable against each other.</p>

          <p><strong>Why two methodologies can disagree significantly:</strong> In testing, commercial retail came out roughly 98% <em>lower</em> cost under the itemized model than the comparison figure (near-zero assumed population means near-zero police/fire demand in the itemized model), while multifamily came out roughly 255% <em>higher</em> (higher population density drives real per-resident service costs the comparison figure doesn't fully capture). That's expected: they're measuring cost two different ways, not one being "more correct." This section is a transparency and cross-check tool, not a replacement for the official rating.</p>
